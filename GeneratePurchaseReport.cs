...
namespace PetrolPumpMGSystem
{
    public partial class GeneratePurchaseReport : Form
    {
        ...
        private void GeneratePurchaseReport_FormClosed(object sender, FormClosedEventArgs e)
        {
            this.Owner.Show();
        }
        ...
        private void button2_Click(object sender, EventArgs e)
        {
            ...
            else
            {
                ...
                if (dt3.Rows.Count != 0)
                {
                    ...
                    {
                        DateTime d1 = Convert.ToDateTime(dt3.Rows[i]["P_DATE"]);
                        DateTime d2 = dateTimePicker1.Value.Date;

                        int prodid = Convert.ToInt32(dt3.Rows[i]["P_PRODID"]);

                        if (prodid == pid && d1.Month == d2.Month && d1.Year == d2.Year)
                        {
                            total = total + Convert.ToDouble(dt3.Rows[i]["P_PAYMENT"]);
                            q = q + Convert.ToDouble(dt3.Rows[i]["P_QUANTITY"]);
                        }
                    }

                    SqlCommand cmd = con.CreateCommand();
                    cmd.CommandType = CommandType.Text;
                    cmd.CommandText = "insert into totalpurchase values('" + total + "','" + q + "','" + DateTime.Now + "','" + pid + "')";
                    cmd.ExecuteNonQuery();
                    MessageBox.Show("Purchase report created");
                }
                else
                {
        ...
        private void GeneratePurchaseReport_Load(object sender, EventArgs e)
        {
            SqlConnection con = new SqlConnection(@"Data Source=(LocalDB)\MSSQLLocalDB;AttachDbFilename=E:\PUCIT\4th sem\DBS\PetrolPumpMGSystem\PetrolPumpMGSystem\PetrolPumpMGSystem\PetrolPump.mdf;Integrated Security=True;Connect Timeout=30");
            con.Open();
